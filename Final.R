...
# 판매액 30, 70 분위수
q70 <- quantile(sales$order_sales, probs = c(0.3))
q30 <- quantile(sales$order_sales, probs = c(0.7))
...
sugg2 <- combinations(length(sales$customer), 2, v=sales$customer, set=TRUE, repeats.allowed=FALSE)
...
sugg4 <- permutations(length(sales$customer), 2, v=sales$customer, set=TRUE, repeats.allowed=FALSE)
